...
slices_stems := $(basename $(notdir $(wildcard slice_meta/*.m4)))
slices_list := $(addsuffix .slice, $(slices_stems))
slice_cmd_names := $(addsuffix p, $(slices_stems))
...
other: hier
	...
	$(m4) -I /usr/share/doc/m4/examples -D ss_cmd_names="$(slice_cmd_names)" profile.sh.m4 >build/profile/simple-slices.sh
